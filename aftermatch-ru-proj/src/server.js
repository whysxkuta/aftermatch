...
function sanitizeServer(row, { includeSecret = false } = {}) {
  if (!row) return null;
  const meta = parseJsonSafe(row.meta_json, {});
  const pingMs = row.last_ping ? (Date.now() - new Date(row.last_ping).getTime()) : null;
  const computedOnline = pingMs !== null && Number.isFinite(pingMs) && pingMs < 20_000;
  return {
    id: row.id,
    name: row.name,
    ip: row.ip,
    port: Number(row.port || 0),
    rcon_password: includeSecret ? (row.rcon_password || "") : undefined,
    sync_token: includeSecret ? (row.sync_token || null) : undefined,
    status: row.status || (Number(row.is_active || 0) ? 'idle' : 'offline'),
    online: computedOnline,
    is_active: Number(row.is_active || 0),
    current_match_id: row.current_match_id || null,
    last_ping: row.last_ping || null,
    manager_version: row.manager_version || null,
    meta
  };
}

function getServerByToken(serverId, syncToken) {
  if (!serverId || !syncToken) return null;
  return db.prepare(`SELECT * FROM servers WHERE id = ? AND sync_token = ?`).get(serverId, syncToken);
}

function buildServerTask(serverRow) {
  if (!serverRow?.current_match_id) return null;
  const match = hydrateMatch(getMatch(serverRow.current_match_id));
  if (!match) return null;
  const mapPool = getTournamentMaps(match.tournament_id);
  return {
    type: 'match_assignment',
    issuedAt: nowIso(),
    server: sanitizeServer(serverRow),
    match: {
      id: match.id,
      status: match.status,
      bo: Number(match.live?.bestOf || match.tournament?.bo || 1),
      scheduledAt: match.scheduled_at || null,
      connectPassword: match.connect_password || '',
      veto: match.veto || null,
      mapPool,
      selectedMaps: Array.isArray(match.live?.selectedMaps) ? match.live.selectedMaps : [],
      currentMap: match.live?.currentMap || (Array.isArray(match.veto?.finalMaps) && match.veto.finalMaps[0]) || null,
      teamA: { id: match.teamA?.id || null, name: match.teamA?.name || 'TBD', tag: match.teamA?.tag || '' },
      teamB: { id: match.teamB?.id || null, name: match.teamB?.name || 'TBD', tag: match.teamB?.tag || '' }
    }
  };
}
...
function hydrateMatch(match) {
  ...
  const veto = parseJsonSafe(match.veto_state_json, null);
  const result = parseJsonSafe(match.result_json, null);
  const live = parseJsonSafe(match.live_json, null);
  return { ...match, tournament: t, teamA, teamB, server: serverRow, veto, result, live, server_ip: match.server_ip || null, server_password: match.server_password || null, scheduled_at: match.scheduled_at || null };
}
  ...
  if (!match) {
    ...
    db.prepare(`
      INSERT INTO matches (
        ...
        bracket_match_id, scheduled_at, server_ip, server_password
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      matchId, row.tournament_id, row.team_a_id, row.team_b_id, "scheduled",
      null, null, now, now,
      row.id, row.scheduled_at || null, row.server_ip || null, row.server_password || null
    );
    ...
    db.prepare(`
      UPDATE matches
      SET team_a_id = ?, team_b_id = ?, scheduled_at = ?, server_ip = ?, server_password = ?, updated_at = ?
      WHERE id = ?
    `).run(row.team_a_id, row.team_b_id, row.scheduled_at || null, row.server_ip || null, row.server_password || null, now, match.id);
...
function publicMatchState(matchId) {
  ...
  return {
    id: m.id,
    status: m.status,
    tournament: { id: m.tournament.id, name: m.tournament.name, mode: m.tournament.mode, bo: m.tournament.bo, mapPool: getTournamentMaps(m.tournament.id) },
...
app.post("/api/tournaments", requireAdmin, (req, res) => {
  ...
  const cleanCheckin = (checkinUntil ?? "").toString().trim() || null;
  const cleanMatchesStart = (matchesStartAt ?? "").toString().trim() || null;
...
app.get("/api/servers", (req, res) => {
  ...
  res.json(rows.map((row) => sanitizeServer(row)));
});
...
app.get("/api/admin/servers", requireAdmin, (req, res) => {
  ...
  res.json(rows.map((row) => sanitizeServer(row, { includeSecret: true })));
});
...
app.post("/api/servers", requireAdmin, (req, res) => {
  ...
  const id = nanoid(10);
  const syncToken = nanoid(24);
  const meta = {
    region: body.region || null,
    launch: body.launch || null,
    rconHost: body.rconHost || ip,
    boModes: Array.isArray(body.boModes) ? body.boModes : [1,3,5]
  };
  db.prepare(`INSERT INTO servers (id, name, ip, port, rcon_password, is_active, status, sync_token, last_ping, current_match_id, meta_json, manager_version) VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?, NULL, ?, ?)`)
    .run(id, name, ip, port, (body.rcon_password || '').toString(), (body.status || 'offline').toString(), syncToken, null, JSON.stringify(meta), body.managerVersion || null);
  res.json({ ok: true, server: sanitizeServer(getServerRow(id), { includeSecret: true }) });
});

app.patch("/api/admin/servers/:id", requireAdmin, (req, res) => {
  const row = getServerRow(String(req.params.id));
  if (!row) return res.status(404).json({ error: "Server not found" });
  const body = req.body || {};
  const nextMeta = {
    ...parseJsonSafe(row.meta_json, {}),
    region: body.region ?? parseJsonSafe(row.meta_json, {}).region ?? null,
    launch: body.launch ?? parseJsonSafe(row.meta_json, {}).launch ?? null,
    rconHost: body.rconHost ?? parseJsonSafe(row.meta_json, {}).rconHost ?? row.ip,
    boModes: Array.isArray(body.boModes) ? body.boModes : (parseJsonSafe(row.meta_json, {}).boModes || [1,3,5])
  };
  db.prepare(`UPDATE servers SET name=?, ip=?, port=?, rcon_password=?, status=?, is_active=?, meta_json=? WHERE id=?`)
    .run(
      (body.name || row.name).toString().trim(),
      (body.ip || row.ip).toString().trim(),
      Number(body.port || row.port || 0),
      (body.rcon_password ?? row.rcon_password ?? '').toString(),
      (body.status || row.status || 'offline').toString(),
      body.is_active === undefined ? Number(row.is_active || 0) : (body.is_active ? 1 : 0),
      JSON.stringify(nextMeta),
      row.id
    );
  res.json({ ok: true, server: sanitizeServer(getServerRow(row.id), { includeSecret: true }) });
});

app.post("/api/admin/servers/:id/rotate-token", requireAdmin, (req, res) => {
  const row = getServerRow(String(req.params.id));
  if (!row) return res.status(404).json({ error: "Server not found" });
  const syncToken = nanoid(24);
  db.prepare(`UPDATE servers SET sync_token = ?, last_ping = NULL, status = 'offline' WHERE id = ?`).run(syncToken, row.id);
  res.json({ ok: true, server: sanitizeServer(getServerRow(row.id), { includeSecret: true }) });
});
...
app.delete("/api/admin/servers/:id", requireAdmin, (req, res) => {
  const row = getServerRow(String(req.params.id));
  if (!row) return res.status(404).json({ error: "Server not found" });
  db.prepare(`UPDATE matches SET server_id = NULL, connect_password = NULL, updated_at = ? WHERE server_id = ?`).run(nowIso(), row.id);
...
app.post("/api/servers/register", (req, res) => {
  const body = req.body || {};
  const name = (body.name || '').toString().trim();
  const ip = (body.ip || body.host || req.ip || '127.0.0.1').toString().replace(/^::ffff:/, '').trim();
  const port = Number(body.port || 0);
  if (!name || !port) return res.status(400).json({ error: 'Bad payload' });

  const existing = body.serverId ? getServerRow(String(body.serverId)) : db.prepare(`SELECT * FROM servers WHERE name = ? AND port = ?`).get(name, port);
  const id = existing?.id || nanoid(10);
  const syncToken = existing?.sync_token || nanoid(24);
  const prevMeta = parseJsonSafe(existing?.meta_json, {});
  const meta = {
    ...prevMeta,
    currentMap: body.currentMap || prevMeta.currentMap || null,
    boModes: Array.isArray(body.boModes) ? body.boModes : (prevMeta.boModes || [1, 3, 5]),
    region: body.region || prevMeta.region || null,
    launch: body.launch || prevMeta.launch || null,
    rconHost: body.rconHost || prevMeta.rconHost || ip,
    lastRconOk: body.lastRconOk ?? prevMeta.lastRconOk ?? null
  };

  if (existing) {
    db.prepare(`UPDATE servers SET name=?, ip=?, port=?, rcon_password=?, is_active=1, status=?, last_ping=?, manager_version=?, meta_json=?, sync_token=? WHERE id=?`)
      .run(name, ip, port, (body.rcon_password || existing.rcon_password || '').toString(), (body.status || existing.status || 'idle').toString(), nowIso(), (body.managerVersion || existing.manager_version || null), JSON.stringify(meta), syncToken, id);
  } else {
    db.prepare(`INSERT INTO servers (id, name, ip, port, rcon_password, is_active, sync_token, status, last_ping, current_match_id, meta_json, manager_version) VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?, NULL, ?, ?)`)
      .run(id, name, ip, port, (body.rcon_password || '').toString(), syncToken, (body.status || 'idle').toString(), nowIso(), JSON.stringify(meta), (body.managerVersion || null));
  }

  res.json({ ok: true, serverId: id, syncToken, server: sanitizeServer(getServerRow(id)) });
});

app.post("/api/servers/heartbeat", (req, res) => {
  const body = req.body || {};
  const server = getServerByToken(String(body.serverId || ''), String(body.syncToken || ''));
  if (!server) return res.status(403).json({ error: 'Bad server credentials' });

  const prevMeta = parseJsonSafe(server.meta_json, {});
  const nextMeta = {
    ...prevMeta,
    currentMap: body.currentMap ?? prevMeta.currentMap ?? null,
    playerCount: body.playerCount ?? prevMeta.playerCount ?? null,
    lastStatusText: body.rawStatus ?? prevMeta.lastStatusText ?? null,
    lastRconOk: body.lastRconOk ?? prevMeta.lastRconOk ?? null,
    boModes: Array.isArray(body.boModes) ? body.boModes : (prevMeta.boModes || [1,3,5]),
    launch: body.launch ?? prevMeta.launch ?? null,
    region: body.region ?? prevMeta.region ?? null,
    rconHost: body.rconHost ?? prevMeta.rconHost ?? server.ip
  };

  db.prepare(`UPDATE servers SET status = ?, current_match_id = COALESCE(?, current_match_id), last_ping = ?, meta_json = ?, manager_version = ?, is_active = 1 WHERE id = ?`)
    .run((body.status || server.status || 'idle').toString(), body.currentMatchId || null, nowIso(), JSON.stringify(nextMeta), body.managerVersion || server.manager_version || null, server.id);

  const fresh = getServerRow(server.id);
  res.json({ ok: true, server: sanitizeServer(fresh), task: buildServerTask(fresh) });
});

app.post("/api/servers/pull-task", (req, res) => {
  const server = getServerByToken(String(req.body?.serverId || ''), String(req.body?.syncToken || ''));
  if (!server) return res.status(403).json({ error: 'Bad server credentials' });
  res.json({ ok: true, task: buildServerTask(server) });
});

app.get("/api/matches/:id/live", (req, res) => {
  const m = hydrateMatch(getMatch(String(req.params.id)));
  if (!m) return res.status(404).json({ error: 'Not found' });
  res.json({ ok: true, live: m.live || null, matchId: m.id });
});
...
app.post("/api/live/update", (req, res) => {
  ...
  const matchId = String(body.matchId || server.current_match_id || '');
  const match = getMatch(matchId);
  if (!match) return res.status(404).json({ error: 'Match not found' });
  if (match.server_id && match.server_id !== server.id) return res.status(409).json({ error: 'Server is not assigned to this match' });

  const previous = parseJsonSafe(match.live_json, {});
  const nextLive = {
    ...previous,
    source: 'rcon',
    updatedAt: nowIso(),
    status: body.status || previous.status || 'live',
    currentMap: body.currentMap ?? previous.currentMap ?? null,
    selectedMaps: Array.isArray(body.selectedMaps) ? body.selectedMaps : (previous.selectedMaps || []),
    bestOf: Number(body.bestOf ?? previous.bestOf ?? 1),
    seriesScore: {
      a: Number(body.seriesScoreA ?? previous.seriesScore?.a ?? 0),
      b: Number(body.seriesScoreB ?? previous.seriesScore?.b ?? 0)
    },
    mapScore: {
      a: Number(body.mapScoreA ?? previous.mapScore?.a ?? 0),
      b: Number(body.mapScoreB ?? previous.mapScore?.b ?? 0)
    },
    players: Array.isArray(body.players) ? body.players : (previous.players || []),
    rawStatus: body.rawStatus ?? previous.rawStatus ?? null,
    extra: body.extra ?? previous.extra ?? null
  };

  db.prepare(`UPDATE matches SET live_json = ?, updated_at = ? WHERE id = ?`).run(JSON.stringify(nextLive), nowIso(), match.id);

  if (body.status === 'live' || body.status === 'ready' || body.status === 'finished') {
    db.prepare(`UPDATE matches SET status = ?, updated_at = ? WHERE id = ?`).run(body.status, nowIso(), match.id);
  }

  const scoreA = Number(body.mapScoreA ?? NaN);
  const scoreB = Number(body.mapScoreB ?? NaN);
  if (match.bracket_match_id && Number.isFinite(scoreA) && Number.isFinite(scoreB)) {
    db.prepare(`UPDATE bracket_matches SET score_a = ?, score_b = ?, updated_at = ? WHERE id = ?`).run(scoreA, scoreB, nowIso(), match.bracket_match_id);
  }

  io.to(`match:${match.id}`).emit('match:state', publicMatchState(match.id));
  res.json({ ok: true });
});
...
app.post("/api/matches/:id/assign-server", requireAdmin, (req, res) => {
  const matchId = req.params.id;
  const { serverId, map, bestOf } = req.body || {};
  const m = hydrateMatch(getMatch(matchId));
  if (!m) return res.status(404).json({ error: "Not found" });
  ...
  const nextLive = {
    ...(m.live || {}),
    currentMap: map || m.live?.currentMap || null,
    selectedMaps: Array.isArray(m.live?.selectedMaps) ? m.live.selectedMaps : [],
    bestOf: Number(bestOf || m.tournament?.bo || 1),
    status: m.live?.status || 'assigned',
    updatedAt: nowIso()
  };

  db.prepare(`UPDATE matches SET server_id = ?, connect_password = ?, live_json = ?, updated_at = ? WHERE id = ?`)
    .run(serverId, pwd, JSON.stringify(nextLive), nowIso(), matchId);
  db.prepare(`UPDATE servers SET current_match_id = ?, status = ?, last_ping = ? WHERE id = ?`)
    .run(matchId, 'busy', nowIso(), serverId);

  io.to(`match:${matchId}`).emit("match:state", publicMatchState(matchId));
  res.json({ ok: true, task: buildServerTask(getServerRow(serverId)) });
});
...
app.get("/api/matches/:id/room", requireAuth, (req, res) => {
  ...
  const userId = req.session.user_id;
  if (!canUserOpenMatchRoom(matchId, userId)) return res.status(403).json({ error: "Нет доступа к комнате матча" });
...
app.patch("/api/tournaments/:id/settings", requireAdmin, (req, res) => {
  ...
  `).run(
    ...
    (body.checkin_from ?? row.checkin_from ?? "").toString().trim() || null,
    (body.checkin_until ?? row.checkin_until ?? "").toString().trim() || null,
    (body.matches_start_at ?? row.matches_start_at ?? "").toString().trim() || null,
...
app.get("/api/version", (req,res)=>res.json({ version:"aftermatch-fix16" }));
...
app.post("/api/tournaments/:id/register-team", requireAuth, (req, res) => {
  ...
  if (Number(tournament.registration_closed || 0) === 1) {
    return res.status(409).json({ error: "Регистрация на турнир закрыта" });
  }
  if (!Number(tournament.early_checkin_open || 0) && tournament.checkin_until && nowIso() > tournament.checkin_until) {
    return res.status(409).json({ error: "Чек-ин уже закрыт" });
  }
...
app.patch("/api/bracket-matches/:id", requireAdmin, (req, res) => {
  ...
  const scheduledAt = (body.scheduled_at ?? row.scheduled_at ?? null) || null;
  ...
  db.prepare(`UPDATE bracket_matches SET team_a_id=?, team_b_id=?, scheduled_at=?, server_ip=?, server_password=?, score_a=?, score_b=?, winner_team_id=?, updated_at=? WHERE id=?`)
    .run(teamAId, teamBId, scheduledAt, serverIp, serverPassword, scoreA, scoreB, winnerTeamId, nowIso(), id);
