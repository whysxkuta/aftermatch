...
async function sendMailSafe({ to, subject, text, html }) {
  ...
  if (!mailer) { console.log(`[mail:skip] ${subject} -> ${to}`); return { skipped: true }; }
...
app.post("/api/auth/send-register-code", async (req, res) => {
  ...
  await sendMailSafe({ to: cleanEmail, subject: "aftermatch.ru — код подтверждения", text: `Код подтверждения: ${issued.code}`, html: `<div style="font-family:Arial,sans-serif;color:#111">Код подтверждения: <b style="font-size:20px">${issued.code}</b></div>` });
  res.json({ ok: true, gmailConfigured: !!getMailer() });
});
...
app.post("/api/auth/request-password-reset", async (req, res) => {
  ...
  if (user) {
    ...
    await sendMailSafe({ to: cleanEmail, subject: "aftermatch.ru — сброс пароля", text: `Код сброса: ${issued.code}`, html: `<div style="font-family:Arial,sans-serif;color:#111">Код сброса: <b style="font-size:20px">${issued.code}</b></div>` });
  }
  res.json({ ok: true });
});
