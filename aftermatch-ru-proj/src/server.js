...
const MAIL_FROM = process.env.MAIL_FROM || GMAIL_USER || "no-reply@aftermatch.ru";
...
async function sendMailSafe({ to, subject, text, html }) {
  ...
  if (!mailer) {
    console.log(`[mail:skip] ${subject} -> ${to} (GMAIL_USER / GMAIL_APP_PASSWORD not configured)`);
    return { ok: false, skipped: true, error: "Почта на сервере не настроена" };
  }
  try {
    const info = await mailer.sendMail({ from: MAIL_FROM, to, subject, text, html });
    console.log(`[mail:sent] ${subject} -> ${to} (${info.messageId || 'no-message-id'})`);
...
app.get("/auth/steam/return", async (req, res) => {
  ...
  rp.verifyAssertion(req, async (err, result) => {
    ...
    return res.redirect(`/auth.html?mode=register&steam=${steamId64}`);
  });
});
...
app.post("/api/auth/register", async (req, res) => {
  const s = getSession(req.cookies?.[SESSION_COOKIE]);
  if (!s || !s.pending_steam_id64) return res.status(400).json({ error: "Сначала привяжи Steam" });
  ...
  db.prepare(`INSERT INTO users (id, nickname, email, email_verified_at, password_hash, first_name, last_name, middle_name, steam_id64, steam_profile_name, role, created_at)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'player', ?)`)
    .run(userId, nickname, String(email).trim().toLowerCase(), nowIso(), hash, firstName, lastName, (middleName || null), s.pending_steam_id64, s.pending_steam_profile_name, nowIso());
...
app.post("/api/auth/send-register-code", async (req, res) => {
  ...
  if (db.prepare(`SELECT 1 FROM users WHERE lower(email) = lower(?)`).get(cleanEmail)) return res.status(409).json({ error: "Почта уже используется" });
  const issued = issueEmailCode(cleanEmail, "register", null, 15);
  const result = await sendMailSafe({ to: cleanEmail, subject: "aftermatch.ru — код подтверждения", text: `Код подтверждения: ${issued.code}`, html: `<div style="font-family:Arial,sans-serif;color:#111">Код подтверждения: <b style="font-size:20px">${issued.code}</b></div>` });
...
app.post("/api/auth/request-password-reset", async (req, res) => {
  const cleanEmail = String(req.body?.email || "").trim().toLowerCase();
  const user = db.prepare(`SELECT id FROM users WHERE lower(email) = lower(?)`).get(cleanEmail);
  if (user) {
    const issued = issueEmailCode(cleanEmail, "reset_password", { userId: user.id }, 15);
    const result = await sendMailSafe({ to: cleanEmail, subject: "aftermatch.ru — сброс пароля", text: `Код сброса: ${issued.code}`, html: `<div style="font-family:Arial,sans-serif;color:#111">Код сброса: <b style="font-size:20px">${issued.code}</b></div>` });
    if (!result.ok) return res.status(500).json({ error: result.error || "Не удалось отправить письмо" });
  }
  res.json({ ok: true });
});
