...
app.get("/api/matches/:id/room", requireAuth, (req, res) => {
  ...
  const admin = !!db.prepare(`SELECT 1 FROM users WHERE id = ? AND role = 'admin'`).get(userId);
  const teamAPlayers = getTeamMembers(state.teamA.id).map(p => ({
    id: p.user_id,
    nickname: p.nickname,
    avatar_url: p.avatar_url || null,
    role: p.role,
    online: !!p.online,
    steam_profile_name: p.steam_profile_name || null
  }));
  const teamBPlayers = getTeamMembers(state.teamB.id).map(p => ({
    id: p.user_id,
    nickname: p.nickname,
    avatar_url: p.avatar_url || null,
    role: p.role,
  ...
  const resultScore = state.result?.score || null;
  const winnerTeamId = state.result?.winnerTeamId || null;
  ...
  res.json({
    ...
    isAdmin: admin,
    captains: {
      A: teamAPlayers.find(p => p.role === 'captain')?.nickname || null,
      B: teamBPlayers.find(p => p.role === 'captain')?.nickname || null
    },
    teamAPlayers,
    teamBPlayers,
    score: {
      a: resultScore ? Number(resultScore.a || 0) : null,
      b: resultScore ? Number(resultScore.b || 0) : null
    },
    winnerSide: winnerTeamId
      ? (winnerTeamId === state.teamA.id ? 'A' : (winnerTeamId === state.teamB.id ? 'B' : null))
      : null
  });
});
...
app.get("/api/notifications", (req, res) => {
  ...
  if (userId) {
    const now = Date.now();
    const upcomingCutoff = new Date(now + 30 * 60 * 1000).toISOString();
    const recentWindowStart = new Date(now - 2 * 60 * 60 * 1000).toISOString();
    ...
    const upcomingMatches = db.prepare(`
      ...
      LIMIT 5
    `).all(nowIso(), upcomingCutoff, userId);
    ...
    for (const m of upcomingMatches) {
      items.push({
        ...
        id: `soon_${m.id}`,
        title: "Скоро матч",
        body: `${m.tournament_name} • ${m.team_a_name} vs ${m.team_b_name} • ${String(m.scheduled_at || "")}`,
    ...
    const roomMatches = db.prepare(`
      ...
      WHERE EXISTS (
        ...
        AND (
          ...
          OR datetime(m.scheduled_at) <= datetime(?)
          OR datetime(m.scheduled_at) >= datetime(?)
        )
      ORDER BY COALESCE(m.scheduled_at, m.created_at) ASC
      LIMIT 5
    `).all(userId, upcomingCutoff, recentWindowStart);
...
app.get("/api/version", (req,res)=>res.json({ version:"aftermatch-fix18" }));
