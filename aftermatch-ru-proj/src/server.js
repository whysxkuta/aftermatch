...
// ====== TEMPLATE ======
const TEMPLATE_PATH = path.join(process.cwd(), "src", "mail-templates", "auth-code.html");
...
function renderMailTemplate({ code }) {
  ...
  return raw
    ...
    .replaceAll("{{TITLE}}", "Давайте подтвердим вашу почту")
    .replaceAll("{{INTRO}}", "Мы отправили вам код подтверждения. Введите его на сайте.")
    .replaceAll("{{CODE_LABEL}}", "КОД ПОДТВЕРЖДЕНИЯ")
    .replaceAll("{{CODE}}", escapeHtml(code))
    .replaceAll("{{EXPIRES_TEXT}}", "Код действует 10 минут.")
...
async function sendMail({ to, code }) {
  if (process.env.MAIL_PROVIDER !== "mailgun") {
    throw new Error("MAIL_PROVIDER != mailgun");
  }

  const html = renderMailTemplate({ code });

  const res = await fetch(
    `${process.env.MAILGUN_BASE_URL || "https://api.mailgun.net"}/v3/${process.env.MAILGUN_DOMAIN}/messages`,
    {
      method: "POST",
      headers: {
        Authorization:
      ...
      body: new URLSearchParams({
        ...
        to,
        subject: "Код подтверждения aftermatch.ru",
        html,
      }),
    }
  );
