...
function hydrateMatch(match) {
  ...
  let live = null;
  try { live = match.live_json ? JSON.parse(match.live_json) : null; } catch { live = null; }
...
function buildServerTask(serverRow) {
  ...
  return {
    ...
    match: {
      id: match.id,
      status: match.status,
      bo: Number(match.tournament?.bo || 1),
      ...
      currentMap: match.live?.currentMap || (Array.isArray(match.veto?.picks?.A) && match.veto.picks.A[0]) || null,
...
function publicMatchState(matchId) {
  ...
  return {
    ...
    serverId: m.server?.id || m.server_id || null,
...
app.post("/api/servers", requireAdmin, (req, res) => {
  const { name, ip, port, rcon_password, status, sync_token } = req.body || {};
  if (!name || !ip || !port) return res.status(400).json({ error: "Bad payload" });
  const id = nanoid(10);
  db.prepare(`INSERT INTO servers (id, name, ip, port, rcon_password, is_active, status, sync_token, last_ping, meta_json) VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?, ?)` )
    .run(id, name, ip, Number(port), rcon_password || "", status || 'idle', sync_token || nanoid(24), nowIso(), JSON.stringify({ boModes:[1,3,5], rconHost: ip }));
  res.json({ id, server: sanitizeServer(getServerRow(id)) });
});
...
app.post("/api/live/update", (req, res) => {
  ...
  if (body.status === 'live' || body.status === 'ready') {
