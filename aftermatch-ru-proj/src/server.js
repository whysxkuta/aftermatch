...
function hydrateMatch(match) {
  ...
  return { ...match, tournament: t, teamA, teamB, server: serverRow, veto, result, server_ip: match.server_ip || null, server_password: match.server_password || null, scheduled_at: match.scheduled_at || null };
}
...
app.get("/api/tournaments/:id/visual-bracket", (req, res) => {
  ...
  const isAdmin = !!(userId && db.prepare(`SELECT 1 FROM users WHERE id = ? AND role = 'admin'`).get(userId));
  const teams = getTournamentTeams(row.id).map(team => ({ id: team.id, name: team.name, tag: team.tag || "" }));
  const bracketMatches = getBracketMatches(row.id).map(m => ({
    id: m.id,
    round_number: m.round_number,
    slot_number: m.slot_number,
    team_a_id: m.team_a_id || null,
    team_b_id: m.team_b_id || null,
    teamA: m.teamA ? { id: m.teamA.id, name: m.teamA.name, tag: m.teamA.tag || "" } : null,
    teamB: m.teamB ? { id: m.teamB.id, name: m.teamB.name, tag: m.teamB.tag || "" } : null,
    score_a: m.score_a,
    score_b: m.score_b,
    winner_team_id: m.winner_team_id || null,
    scheduled_at: m.scheduled_at || null,
    server_ip: m.server_ip || null,
    server_password: m.server_password || null,
    match_id: m.match_id || null,
    room_available: !!(m.match_id && (isAdmin || (userId && canUserOpenMatchRoom(m.match_id, userId))))
  }));
...
app.get("/api/notifications", (req, res) => {
  ...
  if (userId) {
    const memberMatches = db.prepare(`
      ...
      ORDER BY COALESCE(m.scheduled_at, m.updated_at, m.created_at) DESC
      LIMIT 12
    `).all(userId);

    for (const row of memberMatches) {
      const state = publicMatchState(row.id);
      if (!state) continue;
      const side = getUserSideForMatch(row.id, userId);
      const label = `${state.tournament.name} • ${state.teamA.name} vs ${state.teamB.name}`;

      if (side && state.veto && ["veto", "ready"].includes(state.status)) {
        items.push({
          type: "veto_start",
          id: `${row.id}:veto`,
          title: "Veto начался",
          body: `${label}`,
          href: `/match.html?matchId=${row.id}`
        });
      }

      if (canUserOpenMatchRoom(row.id, userId)) {
        items.push({
          type: "match_room",
          id: row.id,
          title: "Комната матча",
          body: `${label}`,
          href: `/match-room.html?id=${row.id}`
        });
      }
    }
  }
