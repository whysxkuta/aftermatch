...
app.get("/api/users/search", (req, res) => {
  try {
    ...
    const users = hasLogin
      ? db.prepare(`
          SELECT id, login, nickname, avatar_url
          FROM users
          WHERE login LIKE ? OR nickname LIKE ?
          LIMIT 20
        `).all(`%${q}%`, `%${q}%`)
      : db.prepare(`
          SELECT id, email as login, nickname, avatar_url
          FROM users
...
// ===== FIXED FRIEND ADD =====
app.post("/api/friends/add", requireAuthApi, (req, res) => {
  try {
    const me = getSession(req.cookies?.[SESSION_COOKIE]);
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?)
    `).run(me.user_id, userId, nowIso());

    db.prepare(`
      INSERT OR IGNORE INTO friends (user_id, friend_id, created_at)
      VALUES (?, ?, ?)
    `).run(userId, me.user_id, nowIso());

    res.json({ ok: true });
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: "ADD_FRIEND_ERROR" });
  }
});

// ===== TEAM ACTIONS =====
app.post("/api/team/leave", requireAuthApi, (req, res) => {
  try {
    const s = getSession(req.cookies?.[SESSION_COOKIE]);
    db.prepare(`UPDATE users SET team_id=NULL WHERE id=?`).run(s.user_id);
    res.json({ ok: true });
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: "LEAVE_TEAM_ERROR" });
  }
});

app.post("/api/team/kick", requireAuthApi, (req, res) => {
  try {
    const s = getSession(req.cookies?.[SESSION_COOKIE]);
    const userId = Number(req.body.userId);

    const captainTeam = db.prepare(`
      SELECT * FROM teams WHERE captain_user_id=?
    `).get(s.user_id);

    if (!captainTeam) {
      return res.status(403).json({ error: "NOT_CAPTAIN" });
    }

    db.prepare(`
      UPDATE users SET team_id=NULL WHERE id=? AND team_id=?
    `).run(userId, captainTeam.id);

    res.json({ ok: true });
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: "KICK_ERROR" });
  }
});
