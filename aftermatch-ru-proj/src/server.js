...
function hydrateBracketMatch(row) {
  ...
  const linkedMatch = row.match_id ? getMatch(row.match_id) : null;
  return {
    ...row,
    teamA,
    teamB,
    winner,
    linked_match_status: linkedMatch?.status || null,
    linked_match_scheduled_at: linkedMatch?.scheduled_at || row.scheduled_at || null
  };
}
...
function ensureVeto(matchId) {
  ...
  if (!veto) {
    ...
    veto = createVetoState(t.bo, mapPool, t.ready_confirm_timeout_seconds || 60);
    saveMatchVeto(matchId, veto);
...
t.bracket_matches = getBracketMatches(t.id).map(m => ({
  ...m,
  can_open_room: !!m.match_id && (!!session?.user_id ? canUserOpenMatchRoom(m.match_id, session.user_id) : false),
  room_url: m.match_id ? `/match-room.html?id=${m.match_id}` : null,
  is_live: ["awaiting_ready", "veto", "ready", "live"].includes(String(m.linked_match_status || ""))
}));
t.room_matches = [];
if (session?.user_id) {
...
app.get("/api/matches/:id/room", (req, res) => {
  ...
  const side = userId ? getUserSideForMatch(matchId, userId) : null;
  const admin = !!(userId && db.prepare(`SELECT 1 FROM users WHERE id = ? AND role = 'admin'`).get(userId));
  const participant = !!(admin || side || (userId && db.prepare(`SELECT 1 FROM team_members WHERE user_id = ? AND team_id IN (?, ?) LIMIT 1`).get(userId, state.teamA.id, state.teamB.id)));
  ...
  res.json({
    ...state,
    serverIp: participant ? state.serverIp : null,
    serverPassword: participant ? state.serverPassword : null,
    connect: participant ? state.connect : null,
    mySide: side,
    isAdmin: admin,
    participant,
