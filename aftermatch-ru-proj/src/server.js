...
app.post('/api/friends/add',(req,res)=>{
 const me=1;
 const {userId}=req.body;
 if(!userId||userId==me) return res.json({error:'INVALID_USER'});
 db.prepare(`INSERT OR IGNORE INTO friends (user1,user2) VALUES (?,?)`).run(me,userId);
 res.json({ok:true});
});

// ==== FIX: TEAM LEAVE ====
app.post('/api/team/leave',(req,res)=>{
 db.prepare(`UPDATE users SET team_id=NULL WHERE id=1`).run();
 res.json({ok:true});
});
