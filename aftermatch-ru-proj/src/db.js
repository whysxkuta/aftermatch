...
try { db.exec("ALTER TABLE bracket_matches ADD COLUMN bo INTEGER"); } catch {}
try { db.exec("ALTER TABLE matches ADD COLUMN bo INTEGER"); } catch {}
try { db.exec("ALTER TABLE tournaments ADD COLUMN registration_until TEXT"); } catch {}
try { db.exec("ALTER TABLE tournaments ADD COLUMN ready_confirm_timeout_seconds INTEGER NOT NULL DEFAULT 60"); } catch {}


// === glass/auth/social migrations ===
try { db.exec("ALTER TABLE users ADD COLUMN email TEXT"); } catch {}
try { db.exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_users_email_ci ON users(lower(email)) WHERE email IS NOT NULL"); } catch {}
try { db.exec("ALTER TABLE users ADD COLUMN email_verified_at TEXT"); } catch {}
try { db.exec("ALTER TABLE users ADD COLUMN nickname_changed_at TEXT"); } catch {}
try { db.exec("ALTER TABLE users ADD COLUMN profile_banner_url TEXT"); } catch {}
try { db.exec("ALTER TABLE users ADD COLUMN intro_seen_at TEXT"); } catch {}
try { db.exec("ALTER TABLE teams ADD COLUMN identity_changed_at TEXT"); } catch {}
try { db.exec("ALTER TABLE teams ADD COLUMN captain_user_id TEXT"); } catch {}
try { db.exec("ALTER TABLE tournaments ADD COLUMN allow_solo_players INTEGER NOT NULL DEFAULT 0"); } catch {}
try {
...
try {
  db.exec("CREATE TABLE IF NOT EXISTS tournament_player_registrations (" +
    "tournament_id TEXT NOT NULL," +
    "user_id TEXT NOT NULL," +
    "created_at TEXT NOT NULL," +
    "confirmed_at TEXT," +
...
// Pending Steam registration links
db.exec(`CREATE TABLE IF NOT EXISTS pending_steam_links (
...
try { db.exec("CREATE INDEX IF NOT EXISTS idx_pending_steam_links_expires ON pending_steam_links(expires_at)"); } catch {}
...
try { ensureColumn("users", "email", "TEXT"); } catch {}
try { ensureColumn("users", "email_verified_at", "TEXT"); } catch {}
try { ensureColumn("users", "avatar_url", "TEXT"); } catch {}
try { ensureColumn("users", "profile_banner_url", "TEXT"); } catch {}
try { ensureColumn("users", "intro_seen_at", "TEXT"); } catch {}
try { ensureColumn("users", "nickname_changed_at", "TEXT"); } catch {}
try { ensureColumn("users", "banned_until", "TEXT"); } catch {}
try { ensureColumn("users", "ban_reason", "TEXT"); } catch {}
try { ensureColumn("teams", "identity_changed_at", "TEXT"); } catch {}
try { ensureColumn("teams", "captain_user_id", "TEXT"); } catch {}
try { ensureColumn("tournaments", "allow_solo_players", "INTEGER NOT NULL DEFAULT 0"); } catch {}
try { db.exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_users_email_ci ON users(lower(email)) WHERE email IS NOT NULL"); } catch {}
