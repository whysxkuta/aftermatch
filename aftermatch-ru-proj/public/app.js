(async function(){
  ...
  const items=[];
  if (me) {
    items.push([profileHref,'Профиль',icons.profile]);
    items.push(['/chats.html','Чаты','<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5h16v10H7l-3 3V5Z"/></svg>']);
  }
  items.push(['/tournaments.html','Турниры',icons.tournaments]);
