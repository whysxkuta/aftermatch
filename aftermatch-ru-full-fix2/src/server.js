...
app.get("/api/version", (req,res)=>res.json({ version:"aftermatch-fix10" }));
...
app.patch("/api/bracket-matches/:id", requireAdmin, (req, res) => {
  ...
  const scoreA = body.score_a === "" || body.score_a === None ? row.score_a : body.score_a;
  const scoreB = body.score_b === "" || body.score_b === None ? row.score_b : body.score_b;
